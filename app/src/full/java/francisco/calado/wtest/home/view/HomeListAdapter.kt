...
class HomeListAdapter(
    ...
    private val homeLoadingListItem: LoadingHomeItem,
    ...
    fun addLoadMore() {
        if (!dataList.contains(homeLoadingListItem)) {
            dataList.add(homeLoadingListItem)
            notifyItemInserted(dataList.size - 1)
        }
    }

    fun removeLoadMore() {
        dataList.remove(homeLoadingListItem)
    }
